...
---
-- Redis 연결 획득 (Traditional Redis only - for backward compatibility)
...
function _M.acquire_redis_connection()
  ...
  if kong and kong.log then
    ...
    if conn_id == 0 then
      kong.log.debug("New traditional Redis connection created")
    else
      kong.log.debug("Reused traditional Redis connection #", conn_id)
    end
  end
...
---
-- Redis 연결 반환 (Traditional Redis only - for backward compatibility)
...
function _M.release_redis_connection(red)
  ...
  if not ok then
    ...
    red:close()
    if kong and kong.log then
      kong.log.warn("Failed to set traditional Redis keepalive: ", err)
    end
  end
end

---
-- Store 생성 (Factory Pattern) with ElastiCache support
-- @param table options Optional configuration overrides
-- @return table Mapping store (Redis/ElastiCache or Memory fallback)
function _M.create_mapping_store(options)
  ...
  if options.use_redis ~= false then
    ...
    local redis_type = options.redis_type or "traditional"
    
    if redis_type == "managed" then
      -- Use ElastiCache connection
      return _M.create_elasticache_store(options)
    else
      -- Use traditional Redis connection
      return _M.create_traditional_redis_store(options)
    end
  end
...
---
-- Create traditional Redis store (existing logic)
-- @param table options Configuration options
-- @return table Redis store or nil
function _M.create_traditional_redis_store(options)
  ...
  if red then
    ...
    if ok then
      kong.log.info("[AWS-MASKER] Traditional Redis store created successfully")
      return store
    else
...
---
-- Create ElastiCache store with SSL/TLS and authentication
-- @param table options Configuration options including ElastiCache settings
-- @return table ElastiCache store or nil
function _M.create_elasticache_store(options)
  local redis_integration = require "kong.plugins.aws-masker.redis_integration"
  
  -- Create ElastiCache-aware Redis integration instance
  local redis_client = redis_integration.new({
    redis_host = options.redis_host or _M.redis_config.host,
    redis_port = options.redis_port or _M.redis_config.port,
    redis_database = options.redis_database or _M.redis_config.database,
    redis_timeout = options.redis_timeout or _M.redis_config.connect_timeout,
    redis_keepalive_pool_size = options.redis_keepalive_pool_size or _M.redis_config.keepalive_pool_size,
    redis_keepalive_timeout = options.redis_keepalive_timeout or _M.redis_config.keepalive_timeout,
    -- ElastiCache specific configuration
    redis_type = "managed",
    redis_ssl_enabled = options.redis_ssl_enabled,
    redis_ssl_verify = options.redis_ssl_verify,
    redis_auth_token = options.redis_auth_token,
    redis_user = options.redis_user,
    redis_cluster_mode = options.redis_cluster_mode,
    redis_cluster_endpoint = options.redis_cluster_endpoint
  })
  
  -- Validate ElastiCache configuration
  local config_ok, config_err = redis_client:validate_elasticache_config()
  if not config_ok then
    kong.log.err("[ElastiCache] Configuration validation failed: ", config_err)
    return nil
  end
  
  -- Test ElastiCache connection
  local health_ok, health_err = redis_client:health_check()
  if not health_ok then
    kong.log.err("[ElastiCache] Health check failed: ", health_err)
    return nil
  end
  
  -- Create ElastiCache store
  local store = {
    type = STORE_TYPE_REDIS,
    redis_type = "managed",
    redis_client = redis_client,
    prefix = options.prefix or _M.redis_config.prefix,
    ttl = options.ttl or _M.redis_config.default_ttl,
    
    -- ElastiCache-specific configuration
    ssl_enabled = options.redis_ssl_enabled,
    cluster_mode = options.redis_cluster_mode,
    
    -- Enhanced methods for ElastiCache
    acquire_connection = function()
      return redis_client:get_connection()
    end,
    release_connection = function(red)
      return redis_client:return_connection(red)
    end,
    health_check = function()
      return redis_client:health_check()
    end,
    get_stats = function()
      return redis_client:get_elasticache_stats()
    end,
    benchmark = function(iterations)
      return redis_client:benchmark_connection_performance(iterations)
    end,
  ...
  kong.log.info("[AWS-MASKER] ElastiCache store created successfully", {
    ssl_enabled = store.ssl_enabled,
    cluster_mode = store.cluster_mode
  })
