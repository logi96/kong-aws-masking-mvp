...
function AwsMaskerHandler:new()
  local instance = {
    ...
    redis_client = nil,   -- Redis integration instance
    config = {
...
function AwsMaskerHandler:access(conf)
  ...
  if not self.redis_client and conf.use_redis then
    local redis_config = {
      redis_host = conf.redis_host or self.config.redis_host,
      redis_port = conf.redis_port or self.config.redis_port,
      redis_password = conf.redis_password or self.config.redis_password,
      redis_database = conf.redis_database or self.config.redis_database,
      redis_timeout = conf.redis_timeout or self.config.redis_timeout,
      redis_keepalive_pool_size = conf.redis_keepalive_pool_size or self.config.redis_keepalive_pool_size,
      redis_keepalive_timeout = conf.redis_keepalive_timeout or self.config.redis_keepalive_timeout
    }
    self.redis_client = redis_integration.new(redis_config)
    
    -- Test Redis connection
    local redis_ok, redis_err = self.redis_client:health_check()
    if not redis_ok then
      kong.log.err("[Redis] Failed to connect: ", redis_err)
  ...
  if not self.mapping_store then
    local store_options = {
      ...
      redis_client = self.redis_client  -- Pass Redis client to masker
    }
...
function AwsMaskerHandler:body_filter(conf)
  ...
  if chunk and chunk ~= "" then
    ...
    for masked_id, original_value in pairs(kong.ctx.shared.aws_memory_mappings) do
      chunk = string.gsub(chunk, masked_id, original_value)
    end
