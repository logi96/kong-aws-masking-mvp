...
# 강력한 시스템 프롬프트와 함께 API 호출
REQUEST_JSON=$(jq -n --arg msg "$ORIGINAL_TEXT" '{
  model: "claude-3-5-sonnet-20241022",
  messages: [
    {
      role: "system",
      content: "CRITICAL INSTRUCTION: You MUST return EXACTLY the same text that the user provides, character by character, without ANY modifications, interpretations, or additions. Do NOT add any explanations, comments, or formatting. Simply echo back the EXACT input text. This is a test requirement that MUST be followed precisely."
    },
    {
      role: "user",
      content: $msg
...
if [ "$ORIGINAL_TEXT" = "$CLAUDE_TEXT" ]; then
  echo "✅ 성공: 원본 텍스트와 최종 출력이 동일합니다!"
else
  echo "❌ 실패: 원본 텍스트와 최종 출력이 다릅니다."
  echo ""
  echo "원본:"
  echo "$ORIGINAL_TEXT"
  echo ""
