...
# REMOVED - Wrong pattern: RESPONSE=$(curl -s -w "\n%{http_code}" -X POST http://localhost:3000/analyze-claude \
...
# REMOVED - Wrong pattern: CRITICAL_RESPONSE=$(curl -s -X POST http://localhost:3000/analyze-claude \
    -H "Content-Type: application/json" \
