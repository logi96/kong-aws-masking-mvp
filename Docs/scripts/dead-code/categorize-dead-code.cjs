...
class DeadCodeCategorizer {
  ...
  async categorize(tsPruneOutputPath) {
  ...
  determineType(item) {
    ...
    // Mock/Test utilities
    if (file.includes('test-doubles') || file.includes('mocks') || name.startsWith('createMock')) {
      return 'test-util';
    }
...
module.exports = { DeadCodeCategorizer };